...
def Proximity(data,domain1,domain2,domainKi2,domainPhi,p,N):
    """
    ...
    for i in range(len(data)):
        ...
        ki2 = np.random.uniform(domainKi2[0],domainKi2[1],N)
        ...
        xi = np.linspace(data.loc[i,'x']*0.3,data.loc[i,'x']*1.3,N)
        ...
        R0,R1,R2 =  ratios(data.loc[i,'x'], data.loc[i,'z'], data.loc[i,'Q2'], data.loc[i,'pT'],Mh,M,zeta,xi,ki2,kf,kT,kibT,deltakT,phi)
